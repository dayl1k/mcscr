...
$status = new Query(gethostbyname($ip),$port); // call the class
$status->connect();
$response=$status->get_info(); // when you dont have the default port

if(!$response) {
	echo $status->get_error()."\n";
     echo '<i style="color:red;" class="fa fa-times"></i> ' . 'Server je ' . '<font color="red"><b>Offline</b></font><br />';
     echo '<i style="color:red;" class="fa fa-child"></i> ' . 'Počet hráčů není k dispozici.<br />';
}else{
    echo '<i style="color:green;" class="fa fa-check"></i> ' . 'Server je ' . '<font color="green"><b>Online</b></font><br />';
    echo '<i style="color:green;" class="fa fa-child"></i> ' . 'Aktuální počet hráčů na serveru: <b>' . $response['numplayers'].'/'.$response['maxplayers'] . '</b><br />';
}
if ($port == 25565){
    echo '<i class="fa fa-connectdevelop"></i> ' . '<b>Server IP:</b> ' . $ip;
}else{
	echo '<i class="fa fa-connectdevelop"></i> ' . '<b>Server IP:</b> ' . $ip . ':' . $port;
}